...
// ─── SVG Icons ──────────────────────────────────────────────────────────────
const LocationIcon = () => (
  <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 flex-shrink-0">
    <path d="M8 1.5a5 5 0 0 1 5 5C13 10.5 8 14.5 8 14.5S3 10.5 3 6.5a5 5 0 0 1 5-5z"/>
    <circle cx="8" cy="6.5" r="2"/>
  </svg>
);

const PhoneIcon = () => (
  <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-3.5 h-3.5 flex-shrink-0">
    <path d="M3 2h2.5l1 3-1.5 1a9 9 0 0 0 4 4l1-1.5 3 1V13a1 1 0 0 1-1 1C5.5 14 2 9 2 3a1 1 0 0 1 1-1z"/>
  </svg>
);
...
const DefaultBadgeIcon = () => (
  <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-3 h-3">
    <path d="M8 2a4.5 4.5 0 0 1 4.5 4.5C12.5 10 8 14 8 14S3.5 10 3.5 6.5A4.5 4.5 0 0 1 8 2z"/>
...
function AddressCard({ address, onEdit, onDelete, onSetDefault, isDeleting }) {
  ...
  return (
    <div className={`bg-white rounded-2xl p-5 border transition-all hover:shadow-lg hover:-translate-y-0.5 ${
      address.is_default 
        ? 'border-primary/60 shadow-md ring-1 ring-primary/10' 
        : 'border-outline-variant/20'
    }`}>
      {/* Header with Avatar, Name, and Default Badge */}
      <div className="flex items-center gap-3 mb-3.5">
        <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-sm flex-shrink-0">
          {getInitials(address.full_name)}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {address.is_default && (
              <div className="inline-flex items-center gap-1 bg-primary/10 text-primary text-[10px] font-semibold px-2 py-0.5 rounded-full whitespace-nowrap flex-shrink-0">
                <DefaultBadgeIcon />
                Default
              </div>
            )}
      ...
      {/* Divider */}
      <hr className="border-outline-variant/10 my-3.5" />

      {/* Address */}
      <div className="flex gap-2.5">
        <LocationIcon />
        <div className="text-xs text-[#3d4258] leading-relaxed flex-1 min-w-0">
      ...
      {/* Actions */}
      <div className="flex items-center gap-2 mt-4 pt-3.5 border-t border-outline-variant/10">
        <button
          onClick={() => onEdit(address)}
          className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-primary/10 text-primary text-xs font-medium hover:bg-primary/20 transition-colors"
        >
          <EditIcon />
          Edit
        </button>
        <button
          ...
          disabled={isDeleting}
          className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-rose-50 text-rose-600 text-xs font-medium hover:bg-rose-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <DeleteIcon />
        ...
        {!address.is_default && (
          <button
            ...
            className="ml-auto inline-flex items-center gap-1 text-on-surface-variant/60 text-xs font-medium hover:text-primary transition-colors"
          >
            <SetDefaultIcon />
            Set as default
          </button>
...
export default function AddressesPage() {
  ...
  const fetchAddresses = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        const sorted = (data.data || []).sort((a, b) => (a.is_default ? -1 : b.is_default ? 1 : 0));
        setAddresses(sorted);
      }
    } catch (err) {
      console.error(err);
  ...
  const handleSaved = (savedAddress, isEdit) => {
    if (isEdit) {
      if (savedAddress.is_default) {
        setAddresses((prev) =>
          prev.map((a) => (a.id === savedAddress.id ? savedAddress : { ...a, is_default: false }))
        );
      } else {
        setAddresses((prev) => prev.map((a) => (a.id === savedAddress.id ? savedAddress : a)));
      }
    ...
    setAddresses((prev) => [...prev].sort((a, b) => (a.is_default ? -1 : b.is_default ? 1 : 0)));
    setShowModal(false);
    setEditingAddress(null);
  };
  ...
  return (
    <div className="min-h-screen bg-[#FAF6F0]/30 text-on-surface flex flex-col font-body">
      ...
      <main className="flex-grow max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 pt-24 sm:pt-28 pb-16 w-full">
        {/* Header */}
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-headline font-bold text-[#242926] mt-1">
              My Addresses
            </h1>
          </div>
          ...
          <button
            onClick={openAdd}
            className="flex items-center gap-1.5 px-5 py-2.5 bg-primary text-white text-xs font-bold rounded-full hover:bg-primary/90 transition-all shadow-md active:scale-95"
          >
            <span className="material-symbols-outlined text-base">add</span>
            Add Address
          </button>
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-white rounded-2xl border border-outline-variant/20 p-5 shadow-xs animate-pulse"
              >
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 rounded-full bg-surface-container" />
                  <div className="flex-1">
                    <div className="h-4 bg-surface-container rounded w-2/3 mb-1.5" />
          ...
          <div className="bg-white border border-outline-variant/20 rounded-2xl p-10 text-center max-w-md mx-auto shadow-xs">
            <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center mx-auto mb-4">
              <span className="material-symbols-outlined text-3xl text-primary/60">location_off</span>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <button
              onClick={openAdd}
              className="rounded-2xl border-2 border-dashed border-outline-variant/30 p-6 flex flex-col items-center justify-center gap-3 hover:border-primary/50 hover:bg-primary/5 transition-all min-h-[240px] group"
            >
              <div className="w-14 h-14 rounded-full bg-primary/5 flex items-center justify-center group-hover:bg-primary/10 transition-colors">
                <span className="material-symbols-outlined text-3xl text-primary/40 group-hover:text-primary/60 transition-colors">
                  add_location_alt
                </span>
              </div>
              <div className="text-center">
                <p className="text-sm font-semibold text-on-surface-variant group-hover:text-primary/80 transition-colors">
                  Add New Address
                </p>
                <p className="text-[10px] text-on-surface-variant/50 mt-0.5">
                  Click to add delivery location
                </p>
              </div>
            </button>
          </div>
